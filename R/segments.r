...
seg_add_relations <- function(segments, streaks_with_relations) {
  ...
  segments %>%
    left_join(streak_relations, by=c("Id.x"="Id")) %>% rename(Relation.x=Relation) %>%
    left_join(streak_relations, by=c("Id.y"="Id")) %>% rename(Relation.y=Relation) %>%
    mutate(Relation=pmax(Relation.x, Relation.y)) %>% select(-Relation.x, -Relation.y)
}

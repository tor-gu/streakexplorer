...
test_that("streaks_get_max_rank_simple basic test", {
  with_mock_db({
    mock_conn <- suppressWarnings(
      dbConnect(RMySQL::MySQL(), dbname="streak_explorer_data")
    )
    ...
    min_year <- 1948
    max_year <- 1949
    actual <- streaks_get_max_rank_simple(lzy_streaks, 3, min_year, max_year,
                                          teams)
    dbDisconnect(mock_conn)
  })
...
test_that("streaks_get_max_rank_by_sampling basic test", {
  with_mock_db({
    ...
    min_year <- 1948
    max_year <- 1949
    actual <- streaks_get_max_rank_by_sampling(lzy_streaks, 3, min_year,
